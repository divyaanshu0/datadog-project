...
function startTracer(configuration, sessionManager, getCommonContext) {
    return {
...
function injectHeadersIfTracingAllowed(configuration, context, sessionManager, getCommonContext, inject) {
    ...
    const traceSampled = (0, sampler_1.isTraceSampled)(session.id, configuration.traceSampleRate);
    const shouldInjectHeaders = traceSampled || configuration.traceContextInjection === browser_core_1.TraceContextInjection.ALL;
    if (!shouldInjectHeaders) {
        return;
    }
    context.traceSampled = traceSampled;
    context.traceId = (0, identifier_1.createTraceIdentifier)();
    context.spanId = (0, identifier_1.createSpanIdentifier)();
    inject(makeTracingHeaders(context.traceId, context.spanId, context.traceSampled, tracingOption.propagatorTypes, getCommonContext, configuration));
}
/**
 * When trace is not sampled, set priority to '0' instead of not adding the tracing headers
 * to prepare the implementation for sampling delegation.
 */
function makeTracingHeaders(traceId, spanId, traceSampled, propagatorTypes, getCommonContext, configuration) {
    ...
    if ((0, browser_core_1.isExperimentalFeatureEnabled)(browser_core_1.ExperimentalFeature.USER_ACCOUNT_TRACE_HEADER) &&
